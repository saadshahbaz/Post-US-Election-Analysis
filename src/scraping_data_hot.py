...
def get_posts(subbreddits, outputfile):
    ...
    data = requests.get(f'http://api.reddit.com/{subbreddit}/hot?limit={num_posts}', headers={'User-Agent': 'windows:requests (by /u/saadshahbaz)'})
    ...
    i=1
    while (i<5):
        data = requests.get(f'http://api.reddit.com/{subbreddit}/hot?limit={num_posts}&after={after}', headers={'User-Agent': 'windows:requests (by /u/saadshahbaz)'})
        x = json.loads(data.content.decode('utf8'))
